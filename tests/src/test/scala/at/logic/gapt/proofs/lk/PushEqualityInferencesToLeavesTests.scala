...
class PushEqualityInferencesToLeavesTests extends Specification with SequentMatchers {
  ...
  "equality right; upper sequent intro. by equality right; aux is not principal" in {
    val proof = ( ProofBuilder
      c OpenAssumption( ( "" -> hof"s=t" ) +: ( "" -> hof"e=r" ) +: Sequent() :+ "" -> hof"A(s)" :+ "" -> hof"B(r)" )
      u ( EqualityRightRule( _, Ant( 1 ), Suc( 1 ), Abs( hov"x", le"B(x):o" ) ) )
      u ( EqualityRightRule( _, Ant( 0 ), Suc( 0 ), Abs( hov"x", le"A(x):o" ) ) ) qed )
    val ( reduction, _ ) = equalityRightReduction( proof.asInstanceOf[EqualityRightRule] ).get
    reduction.conclusion must beMultiSetEqual( proof.conclusion )
    proof mustNotEqual reduction
  }

  "equality right; upper sequent intro. by equality right; aux is principal" in {
    val proof = ( ProofBuilder
      c OpenAssumption( ( "" -> hof"s=t" ) +: ( "" -> hof"s=r" ) +: Sequent() :+ "" -> hof"A(r)" )
  ...
  "equality right; upper sequent intro. by equality left; eq is not principal" in {
    val proof = ( ProofBuilder
      c OpenAssumption( ( "" -> hof"s=t" ) +: ( "" -> hof"e=r" ) +: ( "" -> hof"B(r)" ) +: Sequent() :+ "" -> hof"A(s)" )
  ...
  "equality right; upper sequent intro. by equality left; eq is principal" in {
    val proof = ( ProofBuilder
      c OpenAssumption( ( "" -> hof"s=t" ) +: ( "" -> hof"e=t" ) +: Sequent() :+ "" -> hof"A(s)" )
  ...
  "equality left; upper sequent intro. by equality right" in {
    val proof = ( ProofBuilder
      ...
      u ( EqualityLeftRule( _, Ant( 0 ), Ant( 1 ), Abs( hov"x", le"A(x):o" ) ) ) qed )
    val ( reduction, false ) = equalityLeftReduction( proof.asInstanceOf[EqualityLeftRule] ).get
    reduction.conclusion must beMultiSetEqual( proof.conclusion )
    reduction must beAnInstanceOf[EqualityRightRule]
  }

  "equality left; upper sequent intro. by equality left; eq is principal" in {
    val proof = ( ProofBuilder
  ...
  "equality left; upper sequent intro. by equality left; neither aux nor eq is principal" in {
    val proof = ( ProofBuilder
      c OpenAssumption( ( "" -> hof"s=e" ) +: ( "" -> hof"e=t" ) +: ( "" -> hof"A(s)" ) +: ( "" -> hof"B(e)" ) +: Sequent() )
      u ( EqualityLeftRule( _, Ant( 1 ), Ant( 3 ), Abs( hov"x", le"B(x):o" ) ) )
      u ( EqualityLeftRule( _, Ant( 1 ), Ant( 3 ), Abs( hov"x", le"A(x):o" ) ) ) qed )
    val ( reduction, false ) = equalityLeftReduction( proof.asInstanceOf[EqualityLeftRule] ).get
    reduction.conclusion must beMultiSetEqual( proof.conclusion )
    reduction mustNotEqual proof
  }

  "equality left; upper sequent intro. by equality left; eq in concl. is auxiliary" in {
    val proof = ( ProofBuilder
