...
trait SequentProof[+Formula, This <: SequentProof[Formula, This]] extends DagProof[This] { self: This =>
   ...
   */
  def mainIndices: List[SequentIndex]

  /**
   * The list of main formulas of the rule.
   */
  def mainFormulas: List[Formula] = mainIndices map { conclusion( _ ) }
   ...
   */
  def auxIndices: Vector[List[SequentIndex]]
   ...
   */
  def premises: Vector[Sequent[Formula]] = immediateSubProofs map ( _.conclusion )
   ...
   */
  def auxFormulas: Vector[List[Formula]] = for ( ( p, is ) <- premises zip auxIndices ) yield p( is )

  /**
   * A list of occurrence connectors, one for each immediate subproof.
   */
  def occConnectors: Vector[SequentConnector]
...
trait ContextRule[Formula, This <: SequentProof[Formula, This]] extends SequentProof[Formula, This] { self: This =>
  ...
  override def mainIndices = ( mainFormulaSequent.antecedent.map( _ => true ) ++: contexts.flattenS.map( _ => false ) :++ mainFormulaSequent.succedent.map( _ => true ) ).indicesWhere( _ == true ).toList

  private val contextIndices = for ( ( p, is ) <- premises zip formulasToBeDeleted ) yield p.indicesSequent.delete( is )

  override def occConnectors = for ( i <- contextIndices.indices.toVector ) yield {
